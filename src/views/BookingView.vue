...
function getVerifyCode() {
    axios.get('/api/verifycode?_=' + Date.now())
        .then(response => {
            reswithverifycode.value.verifycode_url = response.data;
        }).catch(error => {
            console.error(error);
        });
}
...
function submit() {
    ...
    axios.put('/api/issue', postJson).then(response => {
        alertInfo.value.info = response.data;
        alertInfo.value.error = '';
    }).catch(error => {
        alertInfo.value.error = error.response.data;
    ...
    <div class="min-h-full flex items-center justify-center">
        <div class="max-w-lg gap-12 justify-center lg:flex lg:max-w-none">
            <div class="max-w-lg space-y-3 mx-4">
                ...
                <p class="text-base-content"> &ensp;&ensp;&ensp;&ensp;电脑义务维修中心于2015年9月创办以来, 是计算机学院学生党支部旗下的一支公益性特色服务团队,
                    以电脑维修工作为重点, 以丰富校园科技文化为己任. 为切实弘扬志愿服务精神, 本中心无偿为全校师生提供电脑义务维修服务. </p>
                <p class="text-base-content"> &ensp;&ensp;&ensp;&ensp;电脑义务维修中心为弘扬志愿服务精神, 无偿为师生提供电脑义务维修服务. 维修前,
                    维修人员会提前告知可能存在的风险, 机主在自行考虑后决定是否交由维修人员进行维修. 若因静电等不可抗力因素导致维修物品损坏或数据丢失等情况, 本中心概不承担任何赔偿责任. 请确定前往维修的师生们,
                    已充分了解并同意此免责声明, 感谢您的理解与支持! </p>
            ...
            <div class="max-w-lg space-y-2 mx-4">
                ...
                <div class="flex items-center gap-x-2 w-full">
                    ...
                    <InputVerify label="验证码" altLabel="不区分大小写" v-model="reswithverifycode.verifycode"
                ...
                <div class="flex items-end justify-between">
                    <div class="flex flex-col space-y-4">
                        <label class="flex cursor-pointer items-center">
                            ...
                            <div class="my-auto font-medium text-base-content text-sm lg:hidden flex"> 我已阅读并同意上侧条款
                            </div>
                            <div class="my-auto font-medium text-base-content text-sm hidden lg:flex"> 我已阅读并同意左侧条款
                            </div>
                        </label>
                ...
                <div>
                    <div v-if="alertInfo.info" class="alert alert-success mt-4" role="alert">{{ alertInfo.info }}</div>
                    <div v-if="alertInfo.error" class="alert alert-error mt-4" role="alert">{{ alertInfo.error }}</div>
                </div>
            </div>
        </div>
    </div>

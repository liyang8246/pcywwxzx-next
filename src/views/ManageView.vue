...
const issueList = ref([])
const getIssueList = () => {
    axios.get('/api/issue?passwd=' + passwd.value).then(res => {
...
const toggleIssue = (issueId) => {
    axios.post(`/api/issue?passwd=${passwd.value}&id=${issueId}`)
    .then(res => {
        getIssueList()
    }).catch(error => {
        console.error(error)
...
const deleteIssue = (issueId) => {
    axios.delete(`/api/issue?passwd=${passwd.value}&id=${issueId}`)
    .then(res => {
        getIssueList()
    }).catch(error => {
        console.error(error)
    ...
    <div class="min-h-full flex flex-col items-center justify-center">
        ...
        <div class="flex justify-center items-center" v-show="issueList.length">
            <table class="table table-sm sm:table">
                ...
                <tbody>
                    <TableItem @toggle-issue="toggleIssue(issue.id)" @delete-issue="deleteIssue(issue.id)" v-for="issue in issueList" :issue="issue"/>
                </tbody>
            </table>
        </div>
    </div>
